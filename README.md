...
- Fighter profiles with portraits, nationality, records, rankings, detailed skill sheets, camp info, morale, annual overall peaks, and fight history. Intentional duplicate career snapshots use a compact `(D)` marker after the display name instead of exposing internal `Legend`, `FA`, or `BAMMA` suffixes; durable `fighter_id` values still distinguish the records.
