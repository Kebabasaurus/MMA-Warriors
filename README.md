...
- Matchmaking keeps Current Fight Card and its action buttons on-screen: the card sits beside Available Fighters on wide displays and moves above them on narrow displays. Its empty state explains how to add the first bout, and every fight keeps slot, matchup, weight, hype, build, fatigue, and medical-return information visible without page-level horizontal scrolling. The full 20-column fighter table remains available through its clearly labelled table scrollbar.
...
- World regions with cities, local popularity, economy, drug-testing accuracy, venues, and promotional benefits. Canada's selectable event and generated-fighter locations include the Ontario cities of Belleville and Kingston.
