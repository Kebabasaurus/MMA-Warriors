...
- Quick Load and Save Manager slot loads show a themed, modal please-wait panel with named phases and progress while large career files are read, rebuilt, and refreshed. If Quick Load needs a recovery snapshot, the same panel identifies that recovery attempt before the result is shown.
